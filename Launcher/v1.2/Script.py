...
Config = {'Url': {'PCgameSDK': '', 'Background': '', 'ico': '', 'Launcher': '', 'GuojiFu': ''}, 'Path': {'launcherPath': '', 'gamePath': ''}, 'Account': {'BFu': [{'name': '默认', 'path': ''}], 'GuanFu': [
...
class Event:
    ...
    def mousePressEvent(self, event):
        ...
        if self.MenuStatus != None:
            ...
            if self.m_Position.x() > 231 and self.MenuStatus == "APPFrame":
                Anim.HideFrameAnim(self)
            if self.m_Position.x() > 361 and self.MenuStatus == "FixedFrame":
                Anim.HideFrameAnim(self)
            if self.m_Position.x() > 231 and self.MenuStatus == "SetFrame":
                Anim.HideFrameAnim(self)
...
class Call:
    def SelectGamePath(self):
        ...
        OperationConfig.WriteConfig(ConfigPath , Config)
        self.ui.GamePath_LineEdit.setText(GamePath)
    ...
    def Start(self):
        GamePath = OperationConfig.GetGamePath()
        if GamePath == "":
            pass
